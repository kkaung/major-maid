...
export default function Reviews({ location, ...props }: ReviewsProps) {
    return (
        <section
            ...
            className={cn(props.className, 'relative space-y-6 max-w-full')}
        >
            <div className="space-y-12">
                ...
                <div>
                    <Marquee
                        pauseOnHover
                        className="transform-cpu [--duration:70s]"
                    >
                        {reviews.map(review => (
...
function ReviewCard({ name, body, ...props }: ReviewCardProps) {
    return (
        <div
            className={cn(
                props.className,
                'h-full w-72 min-w-[22rem] leading-tight border rounded-3xl p-6 bg-secondary ml-2'
            )}
        >
            <div className="flex justify-between">
                ...
                <div>
                    <Icons.fullGoogle className="w-6 h-6" />
                </div>
            </div>
        </div>
    );
